...
layout(set = 2, binding = 0) uniform Dynamic {
    mat4 viewMatrix;
...
void main()
{
	...
	vec3 camRight = vec3(dynamic.viewMatrix[0][0], dynamic.viewMatrix[1][0], dynamic.viewMatrix[2][0]);
	vec3 camUp    = vec3(dynamic.viewMatrix[0][1], dynamic.viewMatrix[1][1], dynamic.viewMatrix[2][1]);
	...
	o_type = type;
	gl_Position = global.projectionMatrix * dynamic.viewMatrix * vec4(worldspacePos, 1.0);
}